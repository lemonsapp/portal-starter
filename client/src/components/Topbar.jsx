...
export default function Topbar() {
  ...
  const avatarBg    = ({ avatar_lemon:"#f5e03a", avatar_rocket:"#3b82f6", avatar_globe:"#22c55e", avatar_diamond:"#a78bfa", avatar_fire:"#ff5500", avatar_crown:"#f5e03a" })[avatarKey] || "#f5e03a";
    ...
    <>
      <style>{`
        ...
        .tb-shell::before {
          ...
          background: linear-gradient(90deg, transparent, rgba(245,224,58,.4), rgba(255,85,0,.4), rgba(245,224,58,.4), transparent);
        ...
        .tb-logo:hover { text-shadow: 0 0 20px rgba(245,224,58,.4); }
        .tb-logo .y { color: #f5e03a; }
        .tb-logo .dot { width:7px; height:7px; background:#ff5500; border-radius:50%; animation:tbpulse 2s ease-in-out infinite; box-shadow:0 0 10px #ff5500; }
        ...
        .tb-user-btn:hover { background:rgba(245,224,58,0.05); border-color:rgba(245,224,58,0.22); transform:translateY(-1px); }
        .tb-role { font-family:'DM Mono',monospace; font-size:9px; letter-spacing:2px; text-transform:uppercase; padding:3px 9px; background:rgba(245,224,58,0.06); border:1px solid rgba(245,224,58,0.2); color:#f5e03a; font-weight:500; }
        ...
        .tb-drop::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,#f5e03a,#ff5500,#f5e03a); background-size:200% 100%; animation:tbBar 4s linear infinite; }
        ...
        .tb-drop-item::before { content:''; position:absolute; left:0; top:0; bottom:0; width:0; background:linear-gradient(90deg,rgba(245,224,58,.3),transparent); transition:width .3s; }
        .tb-drop-item:hover { color:#f5e03a; padding-left:28px; }
        ...
        .tb-logout:hover { background:rgba(255,85,0,0.08); color:#ff8c2a; border-color:rgba(255,85,0,0.25); }
        ...
        <div className="tb-inner">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
            ...
            <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
              ...
              {notifOpen && (
                <div style={{position:"absolute",right:0,top:"calc(100% + 8px)",width:340,maxHeight:480,background:"rgba(10,10,10,.99)",border:"1px solid rgba(245,224,58,.15)",borderRadius:18,overflow:"hidden",zIndex:9999,boxShadow:"0 20px 60px rgba(0,0,0,.8)",animation:"dropIn .25s cubic-bezier(.34,1.56,.64,1)"}} onClick={e=>e.stopPropagation()}>
                  <div style={{padding:"14px 18px",borderBottom:"1px solid rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    ...
                    {unread > 0 && <button onClick={markAllRead} style={{fontSize:10,fontWeight:800,color:"#f5e03a",background:"none",border:"none",cursor:"pointer",letterSpacing:"1px"}}>Marcar todo leído</button>}
                  </div>
                  <div style={{overflowY:"auto",maxHeight:400}}>
                    ...
                    {notifs.map(n => {
                      ...
                      const ANN_CFG = {
                        ...
                        linea:   { emoji: "🌐", color: "#f5e03a", bg: "rgba(245,224,58,.1)", border: "rgba(245,224,58,.3)", label: "Líneas" },
                        ...
                        general: { emoji: "📢", color: "#f5e03a", bg: "rgba(245,224,58,.1)", border: "rgba(245,224,58,.3)", label: "Anuncio" },
                      };
                      const annStyle = isAnn ? (ANN_CFG[annCat] || ANN_CFG.general) : null;
                      return (
                        <a key={n.id} href={n.link||"#"} onClick={(ev)=>{ setNotifOpen(false); if(!n.read){ setNotifs(prev=>prev.map(x=>x.id===n.id?{...x,read:true}:x)); setUnread(u=>Math.max(0,u-1)); fetch(`${API}/api/notifications/read`,{method:"POST",headers:{Authorization:`Bearer ${getToken()}`,"Content-Type":"application/json"},body:JSON.stringify({ids:[n.id]})}).catch(()=>{}); } }} style={{display:"flex",gap:12,padding:"14px 18px",borderBottom:"1px solid rgba(255,255,255,.04)",background:n.read ? "transparent" : (isAnn ? annStyle.bg : "rgba(245,224,58,.04)"),textDecoration:"none",transition:"all .15s",alignItems:"flex-start",borderLeft: isAnn && !n.read ? `2px solid ${annStyle.color}` : "2px solid transparent"}}>
                          {/* Avatar */}
                          <div style={{flexShrink:0,width:40,height:40,borderRadius:isAnn?12:"50%",overflow:"hidden",border:`2px solid ${isAnn ? annStyle.border : "rgba(245,224,58,.2)"}`,background:isAnn ? annStyle.bg : "rgba(245,224,58,.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,filter: isAnn ? `drop-shadow(0 0 6px ${annStyle.color}66)` : "none"}}>
                            {isAnn
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            {isAnn ? (
                              ...
                              <span style={{fontSize:13,fontWeight:800,color:n.actor_name_color||"#f5e03a",textShadow:n.actor_glow>0&&n.actor_glow_color?`0 0 ${n.actor_glow*2}px ${n.actor_glow_color}`:"none",marginRight:6}}>
                                {n.actor_name}
                              </span>
                            ))}
                          ...
                          {!n.read && <div style={{width:8,height:8,borderRadius:"50%",background:isAnn?annStyle.color:"#f5e03a",flexShrink:0,marginTop:6,boxShadow:isAnn?`0 0 8px ${annStyle.color}`:"none"}}/>}
                        </a>
                      );
                    })}
                  </div>
          ...
          {me && (
              <div style={{ position:"relative" }}>
                <div className="tb-user-btn" onClick={e => { e.stopPropagation(); setUserDrop(o => !o); }}>
                  {avatarUrl ? (
                    <div style={{ width:34,height:34,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:"2px solid rgba(245,224,58,0.4)" }}>
                ...
                {userDrop && (
                  <div className="tb-drop" onClick={e => e.stopPropagation()}>
                    <div style={{ padding:"16px 20px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:12,background:"rgba(245,224,58,0.04)" }}>
                      {avatarUrl ? (
                        <div style={{ width:44,height:44,borderRadius:"50%",overflow:"hidden",border:"2px solid rgba(245,224,58,0.4)" }}>
                          <img src={avatarUrl} style={{ width:"100%",height:"100%",objectFit:"cover" }} alt="" />
                        </div>
                      ) : (
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:11,color:"#f5e03a",fontWeight:700 }}><CountUp value={balance}>{balance.toLocaleString()}</CountUp> 🍋 LC</div>
