...
const PALETTE = ["#f5e03a","#ff5500","#ff3300","#22c55e","#60a5fa","#a78bfa","#f472b6","#e11d48","#0ea5e9","#14b8a6","#06b6d4","#ffffff","#ede9e0","#000000"];
const EMOJIS  = ["⚡","🔥","🍋","💎","👑","🌟","🏆","💀","🐋","🌈","❄","🚀","🎯","💜","🖤","⭐","🌀","👾","🎮","🎪"];
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if(effect==="aurora"){
        ...
        [[w*.1,h*.3,color1,.14],[w*.5,h*.2,"#8b5cf6",.11],[w*.8,h*.5,color2,.09],[w*.3,h*.7,"#f5e03a",.07]].forEach(([x,y,c,a])=>{
      ...
      psRef.current.forEach((p,i)=>{
        ...
        else if(effect==="fire"){const fr=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,12+Math.sin(p.phase*3)*6);fr.addColorStop(0,"#f5e03a");fr.addColorStop(.5,"#ff5500");fr.addColorStop(1,"transparent");ctx.fillStyle=fr;ctx.beginPath();ctx.arc(p.x,p.y,14,0,Math.PI*2);ctx.fill();}
...
function LockOverlay({ cost, label, onUnlock, coins, loading }) {
  ...
  return (
    <div style={{position:"absolute",inset:0,borderRadius:"inherit",background:"rgba(4,6,13,.85)",backdropFilter:"blur(4px)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:10,gap:12}}>
      ...
      <div style={{fontSize:11,color:ok?"#f5e03a":"rgba(237,233,224,.35)"}}>🍋 {cost} coins</div>
      {!ok&&<div style={{fontSize:9,color:"rgba(255,85,0,.7)"}}>Te faltan {cost-coins} coins</div>}
      <button onClick={onUnlock} disabled={!ok||loading} style={{fontSize:12,fontWeight:800,letterSpacing:"1.5px",textTransform:"uppercase",padding:"10px 24px",borderRadius:8,border:"none",background:ok?"#f5e03a":"rgba(237,233,224,.1)",color:ok?"#04060d":"rgba(237,233,224,.3)",cursor:ok?"pointer":"not-allowed",opacity:loading?.7:1}}>
...
const BADGE_FX = {
  ...
  fire:{css:()=>`@keyframes fg{0%,100%{box-shadow:0 0 8px #ff5500aa,0 0 20px #ff550055}50%{box-shadow:0 0 16px #ff5500cc}}@keyframes fw{0%,100%{transform:scaleY(1)}50%{transform:scaleY(1.08)}}`,wrap:()=>({animation:"fg 1.8s ease-in-out infinite",background:"linear-gradient(135deg,rgba(255,85,0,.2),rgba(245,224,58,.08))",border:"1.5px solid rgba(255,85,0,.6)"}),text:()=>({background:"linear-gradient(90deg,#f5e03a,#ff5500)",backgroundClip:"text",WebkitBackgroundClip:"text",color:"transparent"}),emoji:"fw 1.5s ease-in-out infinite"},
  ...
  sparkle:{css:()=>`@keyframes sp{0%,100%{box-shadow:0 0 8px #f5e03a55,0 0 20px #ff550033}33%{box-shadow:0 0 12px #a78bfa55}66%{box-shadow:0 0 10px #f5e03a77}}@keyframes spr{from{transform:rotate(0)}to{transform:rotate(360deg)}}`,wrap:()=>({animation:"sp 2.5s ease-in-out infinite",background:"linear-gradient(135deg,rgba(245,224,58,.15),rgba(255,85,0,.1))",border:"1.5px solid rgba(245,224,58,.45)"}),text:()=>({background:"linear-gradient(90deg,#f5e03a,#ff5500,#f5e03a)",backgroundClip:"text",WebkitBackgroundClip:"text",color:"transparent"}),emoji:"spr 4s linear infinite"},
  rainbow:{css:()=>`@keyframes rbf{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}}@keyframes rbg{0%,100%{box-shadow:0 0 10px #f5e03a44}33%{box-shadow:0 0 10px #a78bfa44}66%{box-shadow:0 0 10px #22c55e44}}`,wrap:()=>({background:"linear-gradient(270deg,#f5e03a22,#ff550022,#a78bfa22,#60a5fa22,#22c55e22,#f5e03a22)",backgroundSize:"400% 400%",animation:"rbf 4s ease infinite,rbg 3s ease-in-out infinite",border:"1.5px solid rgba(245,224,58,.3)"}),text:()=>({background:"linear-gradient(90deg,#f5e03a,#ff5500,#a78bfa,#60a5fa,#22c55e)",backgroundClip:"text",WebkitBackgroundClip:"text",color:"transparent"}),emoji:"none"},
...
export default function ProfileStudio({ me, profile, coins, onSave, shopItems=[], chatPowersOwned=[] }) {
  ...
  const [bannerC1,  setBannerC1]  = useState(profile?.banner_color1||"#f5e03a");
  const [bannerC2,  setBannerC2]  = useState(profile?.banner_color2||"#ff5500");
  ...
  const [badgeColor,setBadgeColor]= useState("#f5e03a");
  ...
  const lbl=(t)=>(<div style={{fontSize:9,letterSpacing:"3px",textTransform:"uppercase",color:"#ff5500",marginBottom:14,display:"flex",alignItems:"center",gap:10}}><span style={{width:20,height:1,background:"#ff5500",display:"block"}}/>{t}</div>);
  const tabBtn=(id,icon,label)=>(<button onClick={()=>{setTab(id);setMsg("");}} style={{fontSize:12,fontWeight:800,letterSpacing:"1.5px",textTransform:"uppercase",padding:"8px 16px",borderRadius:8,cursor:"pointer",transition:"all .2s",background:tab===id?"rgba(245,224,58,.12)":"transparent",border:`1px solid ${tab===id?"rgba(245,224,58,.3)":"rgba(237,233,224,.08)"}`,color:tab===id?"#f5e03a":"rgba(237,233,224,.45)"}}>{icon} {label}</button>);

  return (
    <div>
      <style>{`
        @keyframes studioshine{0%,100%{box-shadow:0 0 20px rgba(245,224,58,.06)}50%{box-shadow:0 0 40px rgba(245,224,58,.14)}}
        ...
        input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:#f5e03a;cursor:pointer;box-shadow:0 0 8px rgba(245,224,58,.5)}
      ...
      <div style={{padding:"18px 22px",background:"linear-gradient(135deg,rgba(245,224,58,.06),rgba(255,85,0,.03))",border:"1px solid rgba(245,224,58,.12)",borderRadius:14,marginBottom:20,display:"flex",alignItems:"center",gap:14,animation:"studioshine 3s ease-in-out infinite"}}>
        <div style={{fontSize:32}}>⚡</div>
        <div style={{flex:1}}>
          <div style={{fontSize:22,fontWeight:900,letterSpacing:3,color:"#f5e03a"}}>PROFILE STUDIO</div>
        ...
        <div style={{fontSize:10,color:"#f5e03a",textAlign:"right"}}>
      ...
      {msg&&<div style={{marginBottom:16,padding:"11px 16px",borderRadius:10,background:msg.includes("Error")||msg.includes("error")?"rgba(255,85,0,.1)":"rgba(34,197,94,.1)",border:"1px solid "+(msg.includes("Error")||msg.includes("error")?"rgba(255,85,0,.25)":"rgba(34,197,94,.25)"),color:msg.includes("Error")||msg.includes("error")?"#ff8c2a":"#4ade80",fontSize:14,fontWeight:700}}>{msg}</div>}
              ...
              {card(<>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:14}}>
                  {BANNER_EFFECTS.map(ef=>(
                    <div key={ef.id} className="eff-btn" onClick={()=>setBannerFx(ef.id)} style={{padding:"10px 6px",borderRadius:8,border:`1px solid ${bannerFx===ef.id?"rgba(245,224,58,.4)":"rgba(237,233,224,.07)"}`,background:bannerFx===ef.id?"rgba(245,224,58,.08)":"rgba(255,255,255,.02)",textAlign:"center"}}>
                      <div style={{fontSize:18,marginBottom:3}}>{ef.icon}</div>
                      <div style={{fontSize:7,letterSpacing:"1px",textTransform:"uppercase",color:bannerFx===ef.id?"#f5e03a":"rgba(237,233,224,.35)"}}>{ef.name}</div>
                    </div>
                  ))}
                </div>
                {lbl("Colores")}
                ...
                <button onClick={saveBanner} disabled={saving} style={{width:"100%",height:38,fontSize:12,fontWeight:800,letterSpacing:"1.5px",textTransform:"uppercase",background:"#f5e03a",border:"none",color:"#04060d",borderRadius:8,cursor:"pointer",opacity:saving?.7:1}}>
            ...
            {bannerPresets.length>0&&card(<>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {bannerPresets.map(banner=>{
                  ...
                  return (<div key={banner.key} className="bp" style={{borderRadius:12,overflow:"hidden",border:`1px solid ${owned?"rgba(245,224,58,.25)":"rgba(237,233,224,.07)"}`,background:owned?"rgba(245,224,58,.04)":"rgba(255,255,255,.02)"}}>
                    <div style={{height:60,overflow:"hidden",position:"relative"}}><BannerCanvas effect={data.effect||"stars"} color1={data.color1||"#f5e03a"} color2={data.color2||"#ff5500"} height={60}/></div>
                    <div style={{padding:"10px 12px"}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontSize:12,fontWeight:800,letterSpacing:".5px",textTransform:"uppercase"}}>{banner.name}</span>
                        <span style={{fontSize:8,padding:"1px 6px",borderRadius:100,background:{legendary:"rgba(245,224,58,.1)",epic:"rgba(167,139,250,.1)",rare:"rgba(96,165,250,.1)",common:"rgba(237,233,224,.05)"}[banner.rarity],color:{legendary:"#f5e03a",epic:"#a78bfa",rare:"#60a5fa",common:"#ede9e0"}[banner.rarity]}}>{banner.rarity}</span>
                      </div>
                      {owned?(<button onClick={()=>equipBanner(banner.key)} disabled={saving} style={{width:"100%",height:28,fontSize:10,fontWeight:800,letterSpacing:"1px",textTransform:"uppercase",background:"rgba(245,224,58,.12)",border:"1px solid rgba(245,224,58,.28)",color:"#f5e03a",borderRadius:6,cursor:"pointer"}}>✓ Equipar</button>):(
                        <button onClick={()=>buyBannerPreset(banner)} disabled={saving||balance<banner.cost_coins} style={{width:"100%",height:28,fontSize:10,fontWeight:800,letterSpacing:"1px",textTransform:"uppercase",background:balance>=banner.cost_coins?"#f5e03a":"rgba(237,233,224,.05)",border:"none",color:balance>=banner.cost_coins?"#04060d":"rgba(237,233,224,.3)",borderRadius:6,cursor:balance>=banner.cost_coins?"pointer":"not-allowed"}}>🍋 {banner.cost_coins}</button>
                      )}
                    </div>
                  </div>);
                })}
            ...
            {card(<>
              ...
              <div style={{borderRadius:12,overflow:"hidden",border:"1px solid rgba(237,233,224,.08)",marginBottom:14}}>
                ...
                <div style={{background:"#0b0f1e",padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:46,height:46,borderRadius:"50%",background:"#f5e03a",display:"grid",placeItems:"center",fontSize:22,flexShrink:0}}>🍋</div>
                  <div>
              ...
              <div style={{padding:14,borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid rgba(237,233,224,.06)"}}>
                ...
                {[
                  ...
                  <div key={f.label} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",borderRadius:8,background:f.has?"rgba(34,197,94,.06)":"rgba(255,255,255,.02)",border:`1px solid ${f.has?"rgba(34,197,94,.18)":"rgba(237,233,224,.06)"}`,marginBottom:6}}>
                    ...
                    {!f.has&&f.cost>0&&<span style={{fontSize:10,color:"#f5e03a"}}>🍋 {f.cost}</span>}
                  </div>
                ))}
              </div>
            </>)}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          {chatPowers.length===0?(
            <div style={{textAlign:"center",padding:40,color:"rgba(237,233,224,.3)",fontSize:13}}>
              ...
              <div>No tenés Lemon Powers comprados todavía.</div>
            ...
            {card(<>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:18}}>
                {Object.keys(BADGE_FX).map(id=>(
                  <div key={id} className="eff-btn" onClick={()=>setBadgeFx(id)} style={{padding:"10px 6px",borderRadius:10,border:`1px solid ${badgeFx===id?"rgba(245,224,58,.4)":"rgba(237,233,224,.07)"}`,background:badgeFx===id?"rgba(245,224,58,.08)":"rgba(255,255,255,.02)",textAlign:"center"}}>
                    <div style={{fontSize:9,letterSpacing:"1px",textTransform:"uppercase",color:badgeFx===id?"#f5e03a":"rgba(237,233,224,.4)"}}>{id}</div>
                  </div>
                ))}
              </div>
              {lbl("Color y emoji")}
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:18}}>
                {EMOJIS.map(e=><div key={e} onClick={()=>setBadgeEmoji(e)} style={{width:30,height:30,borderRadius:7,display:"grid",placeItems:"center",fontSize:16,cursor:"pointer",background:badgeEmoji===e?"rgba(245,224,58,.12)":"rgba(255,255,255,.03)",border:`1px solid ${badgeEmoji===e?"rgba(245,224,58,.3)":"rgba(237,233,224,.07)"}`,transition:"all .15s"}}>{e}</div>)}
              </div>
            </>)}
            {card(<>
              {lbl("Datos")}
              <div style={{display:"grid",gap:10}}>
                ...
                <div><div style={{fontSize:9,letterSpacing:"2px",textTransform:"uppercase",color:"rgba(237,233,224,.4)",marginBottom:6}}>Costo (coins)</div><input type="number" value={badgeCost} onChange={e=>setBadgeCost(Math.max(0,parseInt(e.target.value)||0))} min={0} style={{width:"100%",background:"rgba(255,255,255,.05)",border:"1px solid rgba(237,233,224,.12)",borderRadius:8,color:"#f5e03a",fontSize:22,padding:"7px 12px",outline:"none"}}/></div>
              </div>
            </>)}
          </div>
          <div>{card(<>
            ...
            <button onClick={saveBadge} disabled={saving||badgeDone} style={{width:"100%",height:42,fontSize:13,fontWeight:800,letterSpacing:"1.5px",textTransform:"uppercase",background:badgeDone?"rgba(34,197,94,.12)":"#f5e03a",border:badgeDone?"1px solid rgba(34,197,94,.35)":"none",color:badgeDone?"#4ade80":"#04060d",borderRadius:10,cursor:badgeDone?"default":"pointer",opacity:saving?.7:1}}>
