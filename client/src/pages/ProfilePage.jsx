...
const BADGE_CONFIG = {
  ...
  badge_creator: {
    label:"CREATOR", emoji:"🌿",
    glowColor:"#f5e03a55", glowColor2:"#f5e03a11",
    holographic:true,
    textGrad:"linear-gradient(90deg,#f5e03a,#f5e03a,#ff8c00,#f5e03a,#f5e03a)",
    animDur:"2s", particles:true,
    tt:{ name:"Creator", rarity:"✦ LEGENDARIO", desc:"Contribuidor activo de la comunidad. Crea contenido y ayuda a otros miembros de Lemons.", footer:"Otorgado por el equipo Lemons", nameColor:"#f5e03a", rarityColor:"#f5e03a", borderColor:"rgba(245,224,58,.35)", iconBg:"rgba(245,224,58,.15)", iconBorder:"rgba(245,224,58,.3)" },
  },
  badge_legendary: {
...
function PremiumBadge({ badgeKey, name, emoji, fallbackColor="#f5e03a" }) {
    ...
    const iv = setInterval(() => {
      setParts(prev => {
        const np = Array.from({length:3},(_,i)=>({
          ...
          color:["#f5e03a","#f5e03a","#ff5500"][Math.floor(Math.random()*3)],
  ...
  return (
    ...
    >
      {cfg.holographic && <>
        <div style={{ position:"absolute",inset:-1.5,borderRadius:100,background:"linear-gradient(135deg,#f5e03a,#f5e03a,#ff5500,#f5e03a)",backgroundSize:"300% 300%",animation:"holographic 2s linear infinite",zIndex:0 }} />
...
function ItemCard({ item, owned, equipped, onBuy, onEquip, loading, readOnly }) {
  ...
  return (
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} style={{ background:owned?r.bg:"rgba(255,255,255,.02)",border:`1px solid ${owned?r.border:"rgba(237,233,224,.07)"}`,borderRadius:15,padding:"14px 13px",position:"relative",overflow:"hidden",transition:"all .25s",transform:hover?"translateY(-4px)":"none",boxShadow:hover?`0 14px 42px rgba(0,0,0,.45)`:owned?`0 0 20px ${r.border}22`:"none",opacity:owned?1:0.65 }}>
      ...
      {!readOnly && (owned ? (
        <button onClick={()=>onEquip(item)} style={{ width:"100%",height:28,fontSize:8,fontWeight:900,letterSpacing:"1px",textTransform:"uppercase",background:equipped?"rgba(255,85,0,.12)":"rgba(255,255,255,.05)",border:`1px solid ${equipped?"rgba(255,85,0,.28)":"rgba(255,255,255,.1)"}`,color:equipped?"#ff8c2a":"rgba(255,255,255,.45)",borderRadius:8,cursor:"pointer",transition:"all .2s" }}>
          {equipped?"Desequipar":"Equipar"}
        </button>
      ) : (
        <button onClick={()=>onBuy(item)} disabled={loading===item.key} style={{ width:"100%",height:28,fontSize:8,fontWeight:900,letterSpacing:"1px",textTransform:"uppercase",background:item.cost_coins===0?"rgba(34,197,94,.1)":"#f5e03a",border:"none",color:item.cost_coins===0?"#22c55e":"#04060d",borderRadius:8,cursor:"pointer",opacity:loading===item.key?.6:1 }}>
    ...
    <div onClick={onChange} style={{ display:"flex",alignItems:"center",padding:"15px 20px",borderBottom:"1px solid rgba(255,255,255,.04)",cursor:"pointer",transition:"background .2s" }} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,.02)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
        ...
        <div onClick={e=>{e.stopPropagation();onChange();}} style={{ width:46,height:24,borderRadius:100,position:"relative",cursor:"pointer",flexShrink:0,background:value?"rgba(245,224,58,.22)":"rgba(255,255,255,.09)",border:`1px solid ${value?"rgba(245,224,58,.38)":"rgba(255,255,255,.12)"}`,transition:"all .3s" }}>
          <div style={{ position:"absolute",top:3,borderRadius:"50%",width:16,height:16,left:value?23:3,background:value?"linear-gradient(135deg,#f5e03a,#f5e03a)":"rgba(255,255,255,.35)",boxShadow:value?"0 0 10px rgba(245,224,58,.5)":"none",transition:"all .3s cubic-bezier(.34,1.56,.64,1)" }} />
        </div>
      </div>
    </div>
  );
...
function PostCard({ post, onDelete, isOwn }) {
  ...
  const TC = { text:{bg:"rgba(255,255,255,.06)",border:"rgba(255,255,255,.1)",color:"rgba(255,255,255,.5)",label:"📝 POST"}, loot:{bg:"rgba(245,224,58,.1)",border:"rgba(245,224,58,.25)",color:"#fde047",label:"✦ LOOT"}, logro:{bg:"rgba(245,224,58,.1)",border:"rgba(245,224,58,.25)",color:"#f5e03a",label:"🏆 LOGRO"}, envio:{bg:"rgba(96,165,250,.1)",border:"rgba(96,165,250,.25)",color:"#93c5fd",label:"📦 ENVÍO"} };
  ...
  return (
    <div style={{ background:"rgba(255,255,255,.025)",border:"1px solid rgba(255,255,255,.07)",borderRadius:18,padding:18,position:"relative",overflow:"hidden",transition:"all .25s" }} onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,.04)";e.currentTarget.style.borderColor="rgba(255,255,255,.12)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,.025)";e.currentTarget.style.borderColor="rgba(255,255,255,.07)";}}>
      <div style={{ display:"flex",alignItems:"center",gap:11,marginBottom:11 }}>
        ...
        <div>
          <div style={{ fontSize:13,fontWeight:800,background:"linear-gradient(135deg,#f5e03a,#f5e03a)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent" }}>{post.author||post.author_name||"LEMON"}</div>
      ...
      {post.lootItem&&(
        <div style={{ background:"linear-gradient(135deg,rgba(245,224,58,.08),rgba(245,224,58,.04))",border:"1px solid rgba(245,224,58,.2)",borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:14,marginBottom:12 }}>
          ...
          <div>
            ...
            {post.lootItem.rarity&&<div style={{ fontSize:7,fontWeight:900,letterSpacing:"1.5px",textTransform:"uppercase",color:"#f5e03a",marginTop:4,background:"rgba(245,224,58,.1)",border:"1px solid rgba(245,224,58,.25)",borderRadius:100,padding:"2px 8px",display:"inline-block" }}>{post.lootItem.rarity}</div>}
          </div>
        </div>
      )}
        ...
        <div style={{marginTop:12,borderTop:"1px solid rgba(255,255,255,.06)",paddingTop:12}}>
          ...
          {getTok() && (
            <div style={{display:"flex",gap:8,marginTop:8}}>
              ...
              <button onClick={submitComment} disabled={savingComment} style={{height:36,padding:"0 14px",borderRadius:10,border:"none",background:"var(--lemon,#f5e03a)",color:"#000",fontWeight:800,fontSize:12,cursor:"pointer",opacity:savingComment?.6:1}}>
                {savingComment?"...":"→"}
              </button>
            </div>
          )}
...
export default function ProfilePage() {
  ...
  const showToast = (text,color="#f5e03a") => { setToast({text,color}); setTimeout(()=>setToast(null),3500); };
  ...
  async function handleBuy(item) {
    const d=item.data||{}; if(d.grantOnly){showToast("Solo puede ser otorgada por el equipo","#ff5500");return;}
  ...
  function handleEquip(item) {
    ...
    if(item.type==="badge"){const idx=e2.badges.indexOf(item.key);if(idx>=0)e2.badges=e2.badges.filter(b=>b!==item.key);else if(e2.badges.length<4)e2.badges=[...e2.badges,item.key];else{showToast("Máximo 4 insignias","#ff5500");return;}}
    setEquip(e2);
  }
  ...
  return (
    <div style={{background:"#03040c",minHeight:"100vh"}}>
      ...
      <style>{`
        ...
        @keyframes progressGlow{0%,100%{box-shadow:0 0 8px #f5e03a44}50%{box-shadow:0 0 18px #f5e03a88}}
        @keyframes borderSpin{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}}
        @keyframes glowPulse{0%,100%{box-shadow:0 0 20px #f5e03a33}50%{box-shadow:0 0 40px #f5e03a66}}
        ...
        <div className="pf-grid" style={{display:"grid",gridTemplateColumns:"1fr auto",gap:16,alignItems:"start"}}>
          ...
          <div style={{minWidth:0}}>
            ...
            <div className="pf-banner" style={{height:300,borderRadius:"24px 24px 0 0",overflow:"hidden",position:"relative"}}>
              {profile?.profile?.banner_effect&&profile.profile.banner_effect!=="none"?(
                <BannerCanvas effect={profile.profile.banner_effect} color1={profile.profile.banner_color1||"#f5e03a"} color2={profile.profile.banner_color2||"#ff5500"} height={300}/>
              ):(
                <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,#040110,#0a0820,#120605)",backgroundSize:"400% 400%",animation:"holographic 8s ease infinite"}}>
                  <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 30% 50%,#f5e03a18 0%,transparent 60%)"}}/>
              ...
              {/* Editorial bar top */}
              <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,#f5e03a,#ff5500,#f5e03a)",backgroundSize:"200% 100%",animation:"borderSpin 4s linear infinite",zIndex:3}}/>
              {/* Grid sutil */}
              ...
              <div className="pf-banner-eyebrow" style={{position:"absolute",top:24,left:28,zIndex:4}}>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:"3.5px",textTransform:"uppercase",color:"#ff5500",display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                  <span style={{width:28,height:1,background:"#ff5500",display:"inline-block"}}/>
              ...
              {isOwn&&<button onClick={()=>setPanel("studio")} className="pf-banner-edit" style={{position:"absolute",top:24,right:24,zIndex:4,background:"rgba(2,3,7,.7)",backdropFilter:"blur(18px)",border:"1px solid rgba(245,224,58,.25)",color:"#f5e03a",fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:800,letterSpacing:"2px",textTransform:"uppercase",padding:"10px 18px",cursor:"pointer",transition:"all .25s",display:"flex",alignItems:"center",gap:8}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(245,224,58,.12)";e.currentTarget.style.transform="translateY(-1px)";e.currentTarget.style.boxShadow="0 12px 32px rgba(245,224,58,.18)"}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(2,3,7,.7)";e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=""}}>🎨 <span className="pf-banner-edit-text">Editar banner</span></button>}
            </div>
            ...
            <div className="pf-hero" style={{background:"linear-gradient(180deg,rgba(8,10,22,.97),#03040c)",border:"1px solid rgba(255,255,255,.06)",borderTop:"none",padding:"0 28px 24px"}}>
              <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:16}}>
                ...
                <div className="pf-avatar-wrap" style={{position:"relative",display:"inline-block",marginTop:-66,zIndex:10,cursor:storyGroup?"pointer":"default"}}
                  ...
                  <div style={{position:"absolute",inset:-22,borderRadius:"50%",background:"radial-gradient(circle,#f5e03a30 0%,transparent 70%)",animation:"glowPulse 3s ease-in-out infinite",pointerEvents:"none"}}/>
                  ...
                  {storyGroup ? (
                    <div style={{position:"absolute",inset:-6,borderRadius:"50%",padding:3,background:storyGroup.all_viewed
                      ? "rgba(255,255,255,.15)"
                      : "conic-gradient(from 0deg,#a78bfa,#ec4899,#ff5500,#f5e03a,#a78bfa)",
                    ...
                    <div style={{position:"absolute",inset:-6,borderRadius:"50%",padding:3,background:"linear-gradient(135deg,#f5e03a,#f5e03a,#ff5500,#f5e03a,#f5e03a)",backgroundSize:"300% 300%",animation:"borderSpin 3.5s ease infinite"}}>
                      <div style={{width:"100%",height:"100%",borderRadius:"50%",background:"#03040c"}}/>
                    </div>
                  )}
                  {avatarUrl?(
                ...
                {isOwn&&<div className="pf-actions-row" style={{display:"flex",gap:8,paddingBottom:2}}>
                  <Pop as="button" onClick={()=>setEditBio(b=>!b)} style={{height:36,padding:"0 17px",borderRadius:11,fontSize:10,fontWeight:800,letterSpacing:".5px",cursor:"pointer",background:editBio?"rgba(255,85,0,.1)":"rgba(255,255,255,.05)",border:`1px solid ${editBio?"rgba(255,85,0,.28)":"rgba(255,255,255,.1)"}`,color:editBio?"#ff8c2a":"rgba(255,255,255,.5)",display:"flex",alignItems:"center",gap:6}}>✏️ {editBio?"Cancelar":"Editar bio"}</Pop>
                  <Pop as="button" onClick={handleSave} disabled={saving} hoverScale={1.04} style={{height:36,padding:"0 20px",borderRadius:11,fontSize:10,fontWeight:800,letterSpacing:".5px",cursor:saving?"not-allowed":"pointer",background:"linear-gradient(135deg,#f5e03a,#f5e03a 55%,#ff8c00)",border:"none",color:"#000",boxShadow:"0 4px 18px rgba(245,224,58,.42)",display:"flex",alignItems:"center",gap:6}}>✦ {saving?"Guardando...":"Guardar perfil"}</Pop>
                </div>}
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:4}}>
                ...
                {equip.title_key && allItems.find(i=>i.key===equip.title_key) && (()=>{
                  ...
                  return <span style={{fontSize:10,fontWeight:800,letterSpacing:"2px",textTransform:"uppercase",color:td.color||"#f5e03a",background:`${td.color||"#f5e03a"}15`,border:`1px solid ${td.color||"#f5e03a"}30`,padding:"2px 10px",borderRadius:100}}>— {ti.name} —</span>;
                })()}
              </div>
              ...
              {editBio?(
                <div style={{marginBottom:13}}>
                  ...
                  <div style={{display:"flex",gap:7}}>
                    <button onClick={()=>{showToast("✓ Bio guardada","#22c55e");setEditBio(false);}} style={{padding:"6px 14px",background:"rgba(245,224,58,.14)",border:"1px solid rgba(245,224,58,.28)",borderRadius:8,color:"#f5e03a",fontSize:9,fontWeight:900,letterSpacing:"1px",cursor:"pointer"}}>Guardar →</button>
              ...
              {isOwn&&(
                <div style={{marginBottom:12,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.09)",borderRadius:10,padding:"6px 12px"}}>
                    ...
                    {usernameInput!==username&&<button onClick={saveUsername} disabled={savingUsername} style={{background:"rgba(245,224,58,.15)",border:"1px solid rgba(245,224,58,.3)",borderRadius:7,color:"#f5e03a",fontSize:10,fontWeight:900,padding:"3px 10px",cursor:"pointer"}}>{savingUsername?"...":"Guardar"}</button>}
                  </div>
              ...
              {equip.badges&&equip.badges.length>0&&(
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14,position:"relative",zIndex:50,isolation:"isolate"}}>
                  {equip.badges.map(bk=>{
                    ...
                    if(effect)return <BadgePreview key={bk} effect={effect} color={bd.color||"#f5e03a"} emoji={b.emoji} text={b.name} size="small"/>;
                    return <div key={bk} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"4px 12px",borderRadius:100,background:`${bd.color||"#f5e03a"}15`,border:`1px solid ${bd.color||"#f5e03a"}30`,color:bd.color||"#f5e03a",fontSize:9,fontWeight:900,letterSpacing:"1.5px",textTransform:"uppercase"}}>{b.emoji} {b.name}</div>;
                  })}
                </div>
              )}
              ...
              <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                ...
                <div style={{display:"inline-flex",alignItems:"center",gap:10,padding:"10px 16px",background:"linear-gradient(135deg,rgba(245,224,58,.1),rgba(255,85,0,.05))",border:"1px solid rgba(245,224,58,.25)",position:"relative",overflow:"hidden",animation:"glowPulse 4s ease-in-out infinite"}}>
                  ...
                  <div style={{position:"relative",zIndex:1}}>
                    ...
                    <CountUp value={profile?.coins?.balance||0} style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,letterSpacing:"1px",color:"#f5e03a",lineHeight:1.1,marginTop:2,display:"block"}}>{(profile?.coins?.balance||0).toLocaleString()}</CountUp>
                  </div>
                </div>
                ...
                <div style={{flex:1,minWidth:200,maxWidth:280}}>
                  ...
                  <div style={{height:5,background:"rgba(255,255,255,.05)",borderRadius:0,overflow:"hidden",position:"relative",border:"1px solid rgba(255,255,255,.04)"}}>
                    <div style={{height:"100%",width:level==="gold"?"100%":level==="silver"?"60%":"30%",background:`linear-gradient(90deg,${lc.color},${lc.color}cc,#ff5500)`,backgroundSize:"200% 100%",animation:"shimmerBg 3s linear infinite",boxShadow:`0 0 10px ${lc.color}66`}}/>
                  </div>
                </div>
                )}
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 14px",borderLeft:"1px solid rgba(255,255,255,.06)",borderRight:"1px solid rgba(255,255,255,.06)"}}>
                    <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:"1px",color:"#f5e03a",lineHeight:1}}>{followData.followers}</span>
                  ...
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 14px",borderRight:"1px solid rgba(255,255,255,.06)"}}>
                    <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:"1px",color:"#f5e03a",lineHeight:1}}>{followData.following}</span>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderTop:"1px solid rgba(255,255,255,.05)",borderBottom:"1px solid rgba(255,255,255,.05)",background:"linear-gradient(180deg,rgba(245,224,58,.018),transparent)"}}>
              {[
                {ico:"📦",lbl:"Envíos totales", v:String(stats.total_shipments||0), c:"#f5e03a", hide:privacyLoaded&&!privacy.envios},
                ...
                {ico:"🍋",lbl:"Coins ganados",  v:coinsTotal.toLocaleString(),      c:"#f5e03a", hide:privacyLoaded&&!privacy.coins},
              ].map((s,i)=>(
            ...
            <div style={{background:"#070a18",border:"1px solid rgba(255,255,255,.06)",borderTop:"none",borderRadius:"0 0 24px 24px",padding:26,minHeight:400,animation:"slideUp .28s ease"}}>
                  ...
                  {isOwn&&(
                    <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)",borderRadius:18,padding:16,marginBottom:20}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8,paddingTop:10,borderTop:"1px solid rgba(255,255,255,.06)"}}>
                        {["📷","🎬","😊"].map(ic=><div key={ic} style={{width:32,height:32,borderRadius:9,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,cursor:"pointer"}} onClick={()=>showToast("Próximamente","#f5e03a")}>{ic}</div>)}
                        <span style={{fontSize:9,color:"rgba(255,255,255,.2)",marginLeft:4}}>{postInput.length}/280</span>
                        <button onClick={publishPost} style={{marginLeft:"auto",height:34,padding:"0 20px",background:"linear-gradient(135deg,#f5e03a,#f5e03a)",border:"none",borderRadius:10,color:"#000",fontSize:10,fontWeight:900,cursor:"pointer",boxShadow:"0 4px 16px rgba(245,224,58,.38)"}}>Publicar →</button>
                      </div>
                    </div>
                  )}
              ...
              {panel==="perfil"&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:22}}>
                  <div>
                    <div style={{fontSize:8,fontWeight:900,letterSpacing:"4px",textTransform:"uppercase",color:"rgba(255,85,0,.65)",marginBottom:13,display:"flex",alignItems:"center",gap:10}}><span style={{width:18,height:1,background:"linear-gradient(90deg,#ff5500,transparent)"}}/>Avatar</div>
                  ...
                  <div>
                    {["frame","title","badge"].map(type=>{
                      ...
                      return <div key={type} style={{marginBottom:20}}>
                        <div style={{fontSize:8,fontWeight:900,letterSpacing:"4px",textTransform:"uppercase",color:"rgba(255,85,0,.65)",marginBottom:13,display:"flex",alignItems:"center",gap:8}}><span style={{width:18,height:1,background:"linear-gradient(90deg,#ff5500,transparent)"}}/>{type==="frame"?"Marcos":type==="title"?"Títulos":"Insignias"}{type==="badge"&&<span style={{color:"rgba(255,255,255,.2)",fontSize:7}}>({equip.badges?.length||0}/4)</span>}</div>
              ...
              {panel==="logros"&&(
                <div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                    ...
                    <div style={{width:180,height:4,background:"rgba(255,255,255,.07)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${achPct}%`,background:"linear-gradient(90deg,#f5e03a,#f5e03a)",boxShadow:"0 0 10px #f5e03a55",transition:"width 1.5s ease"}}/></div>
                  </div>
                  ...
                  {isOwn&&owned.length>0&&(
                    <div style={{marginTop:32,paddingTop:24,borderTop:"1px solid rgba(255,255,255,.06)"}}>
                      ...
                      {[{type:"avatar",label:"Avatares",icon:"🧑"},{type:"frame",label:"Marcos",icon:"🖼"},{type:"title",label:"Títulos",icon:"📛"},{type:"badge",label:"Insignias",icon:"⚡"}].map(({type,label,icon})=>{
                        const its=allItems.filter(i=>i.type===type&&owned.includes(i.key)); if(!its.length)return null;
                        return <div key={type} style={{marginBottom:20}}>
                          <div style={{fontSize:8,fontWeight:900,letterSpacing:"4px",textTransform:"uppercase",color:"rgba(255,85,0,.65)",marginBottom:13,display:"flex",alignItems:"center",gap:8}}><span style={{width:18,height:1,background:"linear-gradient(90deg,#ff5500,transparent)"}}/>{icon} {label} <span style={{color:"rgba(255,255,255,.18)",fontSize:7}}>({its.length})</span></div>
                ...
                <div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:22,padding:"4px 6px",background:"rgba(255,255,255,.025)",border:"1px solid rgba(245,224,58,.08)",width:"fit-content"}}>
                    {PERSONALIZE_SUBTABS.map(t=>{
                      ...
                      return <button key={t.id} onClick={()=>setSubPanel(t.id)} style={{display:"inline-flex",alignItems:"center",gap:7,padding:"10px 18px",cursor:"pointer",border:"none",fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:800,letterSpacing:"2px",textTransform:"uppercase",background:on?"#f5e03a":"transparent",color:on?"#000":"rgba(240,236,227,.45)",transition:"all .25s"}}>
              ...
              {panel==="privacidad"&&isOwn&&(
                <div>
                  ...
                  <div style={{padding:"13px 18px",background:"rgba(245,224,58,.06)",border:"1px solid rgba(245,224,58,.15)",borderRadius:14,fontSize:11,color:"rgba(255,255,255,.42)",lineHeight:1.6}}>
                    💡 Los cambios se aplican <span style={{color:"#f5e03a",fontWeight:800}}>inmediatamente</span>. Los datos ocultos solo los ves vos.
                  </div>
                  <ConnectedDevicesPanel />
                </div>
              )}

              {msg&&<div style={{marginTop:16,padding:"11px 16px",borderRadius:10,background:"rgba(255,85,0,.1)",border:"1px solid rgba(255,85,0,.25)",color:"#ff8c2a",fontSize:13,fontWeight:700}}>{msg}</div>}
            </div>
          </div>
          ...
          <div className="pf-dock" style={{width:dockOpen?220:58,transition:"width .3s cubic-bezier(.4,0,.2,1)",display:"flex",flexDirection:"column",gap:6,position:"sticky",top:14,flexShrink:0}}>
            ...
            {DOCK_ITEMS.filter(d=>isOwn||["posts","logros"].includes(d.id)).map(item=>(
              ...
              >
                {panel===item.id&&<div style={{position:"absolute",left:0,top:0,bottom:0,width:3,background:"linear-gradient(180deg,#f5e03a,#f5e03a)",borderRadius:"0 2px 2px 0"}}/>}
                <div style={{display:"flex",alignItems:"center",gap:12,padding:13,minHeight:48,whiteSpace:"nowrap",overflow:"hidden"}}>
                  <span style={{fontSize:18,flexShrink:0,width:22,textAlign:"center",filter:panel===item.id?"drop-shadow(0 0 6px #f5e03a)":undefined}}>{item.icon}</span>
                  <span style={{fontSize:10,fontWeight:800,letterSpacing:"1.5px",textTransform:"uppercase",color:panel===item.id?"#f5e03a":"rgba(255,255,255,.45)",opacity:dockOpen?1:0,transform:dockOpen?"translateX(0)":"translateX(-8px)",transition:"all .25s",pointerEvents:"none",flex:1}}>{item.label}</span>
                  {item.badge&&dockOpen&&<span style={{fontSize:9,fontWeight:900,background:"rgba(245,224,58,.15)",border:"1px solid rgba(245,224,58,.25)",borderRadius:100,padding:"1px 7px",color:"#f5e03a",flexShrink:0,opacity:dockOpen?1:0,transition:"all .25s .1s"}}>{item.badge}</span>}
                </div>
              </Pop>
            ))}
          </div>
